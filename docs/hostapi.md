...
> **実装済み(Phase 13、2026-09-06)。追加語彙なし。** `wasm-apps/metronome/` を上表の
> API だけで書き直し、実機で目標値(欠落 0 / clocks÷expected 100.00% / BPM 単峰 /
> 平均間隔 20832.8µs)を確認した(`docs/results/phase13.md`)。
> 仕様どおりに書けなかった箇所はないが、**演奏中のテンポ変更を「次の小節頭」に
> 積む素直な実装はテンポマップを消費し続ける**(エントリを削除・剪定する語彙が
> ないため `SEQCORE_TEMPO_MAX = 32` で頭打ちになる)。メトロノームは
