...
1. **条件 B の正規実行**(負荷 3 分 → 120 へ戻して静粛 3.5 分)を 2 回。
   プローブはペインから切り離して起動し、生存確認してからユーザーに依頼する。
   (B1_aborted の 104.5 秒窓で目標は満たしているが、規定の長さで採り直す)
2. **条件 C**(演奏中テンポ変更 120 → 180 → 120、各 1 分以上)×1。
   `--segments auto` で区間ごとの平均間隔と切替点の外れ値を確認する。
3. **条件 D**(送信側打刻)。`src/components/midi/midi.cpp` に
   `#ifdef PHASE13_TXLOG_TEST` の計測コードを**実装済み**(理想グリッドからの偏差を
   10 秒窓で集計し `PHASE13 TXWIN n=... sd=... mean=... min=... max=... out=...` を出す。
   `analyze.py --txlog` が対応済み)。ビルドは
   `idf.py -DCMAKE_C_FLAGS=-DPHASE13_TXLOG_TEST -DCMAKE_CXX_FLAGS=-DPHASE13_TXLOG_TEST build`
   等で有効化する。判定は **アイドル σ ≤ 30µs / 負荷 σ ≤ 120µs**。
   **A2 の外れ値 2 件が送信側にも現れるか**をここで突き合わせる(同時にプローブでも採る)。
   検証後にコードを削除し、`git grep PHASE13 -- src scripts wasm-apps` が空になることを確認する。
4. **SL MK3 での目視確認**(検知テンポが 120 で安定、条件 C の切替に追従)。
5. **回帰**: `./scripts/device-regress.sh --task phase13-regress`(6 本、free heap 49160 /
   largest block 31744 / WARN・ERROR 0)。clicktest が旧経路のまま動くことも確認。
6. **文書更新**: `docs/architecture.md` §10(ステップ 3 完了日と本ファイルへのポインタ)、
   `docs/hostapi.md` §6 要件 1(実装済み・追加語彙なし)、`docs/status.md`、
   `docs/lessons.md`(SD シードの教訓、ALSA キューの教訓、固定公称でテンポ変動区間を
   判定しない、の 3 点)。
7. **Zenn 用の対比表**(09c の 61% と本フェーズの 0 件を並べる 1 表)を本ファイルに用意する。

### 現在の実機・環境の状態

- 実機: metronome が起動したまま(transport は STOP)。モニタは `esp32-monitor` ペインで
  `captures/phase13/monitor.log` に tee 中。SD は復旧済み(手動コピー)。
- ファームウェアは **PHASE13_TXLOG_TEST なし**の通常ビルドが焼かれている。
- プローブは停止。`unix-build` ペインは再作成済みで正常。
