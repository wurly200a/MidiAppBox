...
- **Phase 10 の設計書一式を作成し、現在ユーザーレビュー中(未承認)**
  ...
  - `docs/architecture-next.md`: `docs/architecture.md` の全面改訂案。
  ...
  - **既存の `docs/architecture.md` と `shared/hostapi_defs.h` は未変更**
    ...
    別ファイルとして提示している)。
  - レビュー論点は `docs/architecture-next.md` §11 に 5 点を列挙。特に
  ...
  - **初版レビュー完了(2026-09-05)**。承認条件とされた 3 点
    ...
    追加論点(STOPPED 中のクロック挙動)の決定も `architecture-next.md` §11
    に記録した。
- **Phase 10 の最終回帰完了(2026-09-05)、合格**。
  ...
  - **残作業**: 設計書の確定反映。`docs/architecture.md` への反映は承認後すぐ
    可能だが、`shared/hostapi_defs.h` への追加は**指示書のゲート「本フェーズでは
    Host API / ABI を変更しない」に抵触する**ため、移行表のステップ 2
    (Phase 11)で行う。
